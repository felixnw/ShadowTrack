...
# --- API URLs ---
ADSB_URL = config.ADSB_URL.format(lat=config.HOME_LAT, lon=config.HOME_LON, range=config.RANGE)
...
# Range in NM to consider when looking for planes
RANGE = config.RANGE
...
def get_closest_plane():
    ...
    try:
        # 1. Pull closest planes from ADS-B API
